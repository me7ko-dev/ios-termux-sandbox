...
## Сесия 10 (2026-09-24) — клавиатура, лента с клавиши, по-бърз десктоп

Оплаквания от телефона: в Desktop таба лентата (Esc/Tab/Ctrl/…) остава
под клавиатурата и не може да се стигне; в терминала бутонът с
клавиатурата на SwiftTerm не я скрива, а я сменя с панел Shift/F1/F2 и
обратно; десктопът е бавен.

**Лента с клавиши** (`ExtraKeysBar.swift`, Termux-стил): закачени отляво
(скрий/покажи клавиатурата, опции) и отдясно (стрелки със задържане),
всичко останало е в хоризонтален скрол между тях — нищо не може да излезе
извън екрана.
- Ubuntu и iOS shell терминалите (`TerminalExtraKeys.swift`): заменя
  аксесоара на SwiftTerm. ⌨︎↓ скрива клавиатурата, тап върху терминала я
  връща. ESC, sticky CTRL/ALT (важат и за следващия клавиш от iOS
  клавиатурата), TAB, HOME/END/PGUP/PGDN, ~ | / -, F1–F12. Стрелките
  спазват application-cursor режима (vim, less) и модификаторите
  (`ESC [1;5A` за Ctrl+↑).
- Desktop: една и съща лента долу, докато клавиатурата е скрита, и **върху
  самата клавиатура** (inputAccessoryView), докато е показана. Клавиши:
  Esc, Tab, sticky Ctrl/Alt/Shift/Super, Del, Home/End/PgUp/PgDn, F1–F12,
  стрелки. Бутон „опции“: резолюция, нулиране на zoom, рестарт на десктопа.
  SwiftUI вече не преоразмерява десктопа при показване на клавиатурата.

**Скорост на десктопа** (от страната на приложението; самият гост е
толкова бърз, колкото позволява QEMU — без JIT е интерпретатор):
- кадрите се правят извън main thread и най-много ~30/s (преди — копие
  на целия framebuffer + нова текстура на main thread при всеки update);
- махнат trilinear филтърът на canvas-а (строеше mipmaps за всеки кадър);
- движението на показалеца се изпраща най-много ~30/s (120 Hz екранът
  пращаше по 120 събития/s, всяко е работа за X сървъра в госта);
- резолюция по избор: Fast 1.25× (1152×528), Balanced 1.5× (1384×632,
  **новото подразбиране**), Sharp 1.75× (1616×744, досегашната). Смяната
  рестартира VNC сесията; вече пуснат десктоп остава на старата резолюция
  до рестарт.

**Не е проверено на телефона** — тук няма Xcode; компилацията се проверява
от CI.


## Сесия 9 (2026-09-23) — първо пускане на телефона без JIT

На iPhone, пуснат с тап на иконата (TCTI интерпретатор, без JIT), първият
boot стига до серийния autologin, но той пада с `Login timed out after 60
seconds` в цикъл — гостът е твърде бавен за стандартните лимити. Оправено в
`Guest/cloud-init/user-data` с `bootcmd` (всеки boot, преди логини):
`LOGIN_TIMEOUT 0`, update-motd скриптовете изключени (Python при всеки вход),
`LoginGraceTime 0` за sshd. `instance-id` → `v2`, за да прочете cloud-init
новия user-data и на вече създаден диск; `seed.iso` е прегенериран.

Също: RAM-ът на госта се ограничава до това, което iOS реално дава (без
increased-memory-limit при безплатен акаунт 2560 MiB водеше до убиване);
snapshot, който не е успял да се зареди, се изтрива при следващия старт
(вместо цикъл от сривове); връщане по време на savevm показва съобщение.

**Не е проверено на телефона.** Най-голямата разлика в скоростта е JIT:
стартиране от Xcode (Run) → бекенд `JIT (TCG)`.
...
`~/.local/bin/claude` в госта и ако го няма, пуска
`Resources/claude-code-install.sh` **във фонов режим** (nohup, SSH
командата се връща за <1 s, терминалът се ползва междувременно):
официалният native инсталатор `curl -fsSL https://claude.ai/install.sh | bash`
(поддържа linux-arm64, без Node.js; npm пакетът иска Node 22+, а jammy
има 12). 3 опита, `flock` срещу двойно пускане, лог в
`~/.cache/ios-claude-install.log`; ако не успее, опитва пак при следващия старт.
В `~/.bashrc` добавя `~/.local/bin` в PATH и еднократна подсказка при вход
(„Claude Code is installed — type: claude“).

Първи вход: `claude` показва линк → докосваш го (терминалът отваря
линкове в Safari) → влизаш → поставяш кода. Или `export ANTHROPIC_API_KEY=...`.

**Проверено тук:** командата се връща веднага, скриптът тръгва отделно,
PATH и подсказката работят в нова SSH сесия, повторните опити работят.
**Не е проверено:** самото сваляне — тази sandbox среда прихваща HTTPS
(самоподписан сертификат) и блокира claude.ai; на телефона връзката е
директна. Документацията препоръчва 4 GB RAM; профилът за 13 Pro Max дава
2.5 GB на госта — ако Claude Code се окаже тежък, това е първото за вдигане.
